...
<div class="col-sm-12">
    ...
    <div class="card">
        ...
        <div class="card-block">
            ...
            @endif
            <a class="btn btn-primary btn-sm" href="{{ route('promo.add') }}">Add</a>            
                ...
                <table id="order-table" class="table table-striped table-bordered nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            {{-- <td>{{ $val->photo }}</td> --}}
                            <td>
                                <form action="{{ route('promo.destroy', $val->id) }}" id="delete_data{{ $val->id }}" method="post">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <div class="btn-group " role="group" data-toggle="tooltip">
                                    <a class="btn btn-success btn-mini" href="{{ route('promo.show', $val->id) }}">                                        
...
function set_active(id) {
    ...
    $.ajax({
        url: "{{ URL('admin/promo') }}/"+ id +"/"+ show,
        type: 'GET',
        success: function(response){
            console.log(response.is_show)
            if (response.is_show === 0) {
                swal("Success", "promo with title "+ response.title +" is not displayed!", "success")
            }else{
                swal("Success", "promo with title "+ response.title +" is displayed!", "success")
            }
        }
    })
}
